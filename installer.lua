if not http then
  error("This installer doesnt work without http API!")
end
print("Please wait...")

local authenticate = _G._GIT_API_KEY and {Authorization = "Bearer ".._G._GIT_API_KEY}
local basaltDL = http.get("https://basalt.madefor.cc/versions/basalt-1.6.4.lua", _G._GIT_API_KEY and authenticate)
...
local function generateTree(sURL)
    ...
    for _,v in pairs(tData) do
        ...
        if not (sName:sub(1,1) == '.' or sName:find(".md") or sName == "installer.lua" or sName == "docs") then
            tTree[sName] = tItem
        end
    end
...
welcome:addLabel()
  ...
  :setText("Welcome! Thank you for choosing this product! With Kristify you are able to set up a shop easily and comfortable, bringing you much success through the currency Krist!")
welcome:addLabel()
  :setPosition(1,6)
  :setSize("parent.w-2","3")
  :setText("To continue with the installation, click on \"Next\" below. To cancel the installation process, press CTRL+T at any time.")
-- Next page
  welcome:addButton()
  :setPosition("parent.w-9","parent.h-2")
  :setSize(10,3)
...
licence:addLabel()
  :setPosition(1,1)
  :setText("Read the licence agreement carefully.")
-- Checkbox
  ...
  :onClick(function()
    if checkbox:getValue() then
      ...
      if nRequiredLen+nLen >= 95 then
        ...
        note:setText("Not enough space left on disk! At least ~"..nRequired.."kb must be free.")
      end
...
destination:addLabel()
  :setPosition(1,1)
  :setText("Choose where Kristify should be installed:")
-- Input frame
...
destination:addLabel()
  :setPosition(1,5)
  :setText("Local Disk Drive:")
destination:addLabel()
  :setPosition(1,8)
  :setText("0b")
local maxMb = destination:addLabel()
  :setPosition("parent.w-self.w+1",8)
...
destination:addLabel()
  :setPosition(nStartPos-1+(nLen<=1 and 1 or 0),7)
  :setForeground(colors.gray)
  :setText("\025Used")
destination:addLabel("_freeLabel")
  :setPosition("parent.w-8",7)
  :setForeground(colors.gray)
  :setText("Free\025")
-- Note
...
:setPosition("parent.w-8","parent.h-2")
:setSize(9,3)
:setText("")
:setBackground(colors.gray)
:setForeground(colors.black)
