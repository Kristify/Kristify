...
local title = base:addLabel()
    :setText("Kristify")
    :setPosition(2, 2)
    :setFontSize(2)

local page = base:addLabel()
    :setText("1/4")
...
content:addFrame():setBackground(colors.lightGray):show(),
    content:addFrame():setBackground(colors.lightGray):hide(),
    content:addFrame():setBackground(colors.lightGray):hide(),
    content:addFrame():setBackground(colors.lightGray):hide(),
    content:addFrame():setBackground(colors.lightGray):hide()
...
local function generateTree(sURL)
  ...
  for _, v in pairs(tData) do
    ...
    if not
        (
        sName:sub(1, 1) == '.' or sName:find(".md") or sName == "installer.lua" or sName:find("basalt") or
            sName == "docs") then
      tTree[sName] = tItem
    end
  end
...
local function installKristify(install)
  ...
  local function listItems(tree, depth)
    ...
    for name, item in pairs(tree) do
      local begin = "   " ..
          ("\149 "):rep(depth - 1) .. (index >= size and '\141' or '\157'):rep(depth - (depth - 1)) ..
          ' '
  ...
  local response, sErr, errResponse = http.get("https://raw.githubusercontent.com/Kristify/Kristify/main/kristify.lua",
    authenticate)
  if not httpError(response, sErr, errResponse) then
    return false
  end
